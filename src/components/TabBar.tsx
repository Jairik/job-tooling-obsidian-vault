// The inline strip of conversation tabs that lives in the top toolbar. Each tab
// shows a status dot (idle / running / done / error), a mode glyph (Ask vs Job),
// an inline-rename input (double-click the name), and a close button.
...
// Map a tab's generation phase to its status-dot modifier. Phases not listed
// here leave the dot at rest (accent on the active tab, neutral otherwise).
  ...
  return (
    <div className="tabstrip">
      {tabs.map((t) => {
        ...
        const dotClass = PHASE_DOT[t.phase] ?? (active ? "idle-active" : "");
        return (
          <div
            key={t.id}
          ...
          >
            ...
            {editing === t.id ? (
              ...
              <span className="t-name" onDoubleClick={() => setEditing(t.id)}>
                {t.name}
              </span>
            )}
            {tabs.length > 1 && (
