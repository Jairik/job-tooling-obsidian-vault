...
`);
  ...
  test("docs workflow deploys the website to GitHub Pages without pushing built docs", async () => {
    ...
    expect(workflow).toContain('bun docusaurus build --out-dir "${{ runner.temp }}/docusaurus-build"');
    expect(workflow).toContain('DOCS_BUILD="${{ runner.temp }}/docusaurus-build"');
    ...
    expect(workflow).toContain('touch "$PAGES_STAGE/.nojekyll"');
    expect(workflow).toContain('echo "PAGES_STAGE=$PAGES_STAGE" >> "$GITHUB_ENV"');
    expect(workflow).toContain("path: ${{ env.PAGES_STAGE }}");
    expect(workflow).toContain("include-hidden-files: true");
    expect(workflow).toContain("pages: write");
    expect(workflow).toContain("id-token: write");
    expect(workflow).not.toMatch(/^\s+env:\s*\n\s+DOCS_BUILD: \$\{\{ runner\.temp \}\}/m);
    expect(workflow).not.toMatch(/^\s+env:\s*\n\s+PAGES_STAGE: \$\{\{ runner\.temp \}\}/m);
